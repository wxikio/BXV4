...
module.exports = {
    name: "voicemuteall",
    description: "Mute tous les utilisateurs present dans le salon vocale",
    aliases: [],
    permissions: [],
    ...
    */
    async execute(client, message, args, db) {
        ...
        for (const member of message.member.voice.channel.members.filter(m => !m.user?.bot).values()){
            try {
                await member.voice.selfMute(true)
                i++;
            }
            catch { false }
        }

        message.channel.send(`${i} membres ont été mutes`)
    },
    /**
     * @param {Client} client
    ...
    */
    async executeSlash(client, interaction, db) {
        ...
        interaction.editReply({ content: `${i} membres ont été mutes` });
    },
    get data() {
